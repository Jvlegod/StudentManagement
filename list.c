...
bool klist_delete(kdouble_list_t *list, klist_node_t *target_node)
{
    klist_node_t *temp = klist_find(list, target_node);
    if (temp == nullptr)
    {
        return false;
    }
    klist_node_t *pre = temp->prev;
    klist_node_t *next = temp->next;
    pre->next = next;
    next->prev = pre;
 ...
 * @return bool
 */
bool klist_insert(kdouble_list_t *list, klist_node_t *node, klist_node_t *target_node)
{
    klist_node_t *temp = klist_find(list, node);
    if (temp == nullptr)
    {
        return false;
    }
